<template>
  <div>
    <div class="nav-container">
              ...
              <el-col>
                <el-dropdown>
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item icon="el-icon-top-left">
                    ...
                    <el-dropdown-item icon="el-icon-key">
                      <a @click="modifyDialog = true">修改密码</a>
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              </el-col>
        ...
        <el-dialog :visible.sync="logindialog" center title="登录" width="30%">
          <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm"  class="demo-ruleForm">
            ...
            <el-form-item>
              ...
              <el-link style="margin-left: 30px" @click="logindialog = false;leaveDialog = true"><span>忘记密码</span></el-link>
            </el-form-item>
          </el-form>
        </el-dialog>
      </div>
      ...
      <div>
        <el-dialog :visible.sync="modifyDialog" center title="修改密码" width="30%">
          <el-form  :model="ruleForm" status-icon :rules="rules" ref="ruleForm"  class="demo-ruleForm">
            <el-form-item prop="code">
              <div style="display:flex;justify-content:space-between">
                ...
                <div>
                  <span @click="getModifyCode(user.email)" v-if="!sendcode">获取验证码</span>
                  <span v-if="sendcode">{{lastNumber}}s</span>
                </div>
            </div>
            </el-form-item>
            <el-form-item prop="password">
              <el-input placeholder="新密码" type="password" v-model="ruleForm.password" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item prop="repass">
              <el-input placeholder="确认密码" type="password" v-model="ruleForm.repass" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item>
                <el-button type="primary" @click="submitModify('ruleForm')">确认修改</el-button>
      ...
      <div>
        <el-dialog :visible.sync="leaveDialog" center title="忘记密码" width="30%">
          <el-form  :model="ruleForm" status-icon :rules="rules" ref="ruleForm"  class="demo-ruleForm">
            <el-form-item prop="leaveEamil">
              <el-input placeholder="邮箱" type="email" v-model="ruleForm.leaveEamil" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item prop="code" v-if="(/^\w+((.\w+)|(-\w+))@[A-Za-z0-9]+((.|-)[A-Za-z0-9]+).[A-Za-z0-9]+$/).test(ruleForm.leaveEamil.trim()) != ''">
              <div style="display:flex;justify-content:space-between">
                <div>
                  <el-input placeholder="验证码" type="text" v-model="ruleForm.code" autocomplete="off"></el-input>
                </div>
                <div>
                  <span @click="getModifyCode(ruleForm.leaveEamil)" v-if="!sendcode">获取验证码</span>
                  <span v-if="sendcode">{{lastNumber}}s</span>
                </div>
            </div>
            </el-form-item>
            ...
            <el-form-item>
                <el-button type="primary" @click="submitLeaveForm('ruleForm')">提交</el-button>
...
export default {
  data() {
    ...
    var validateLeaveEmail = (rule, value, callback) => {
      let reg = /^\w+((.\w+)|(-\w+))@[A-Za-z0-9]+((.|-)[A-Za-z0-9]+).[A-Za-z0-9]+$/
        if (value === '') {
          callback(new Error('请输入邮箱'));
        } else if (!reg.test(value)) {
    ...
    return {
      ...
      rules: {
          ...
          leaveEamil: [
            { validator: validateLeaveEmail, trigger: 'blur' }
          ],
          password: [
    ...
    searchInputSubmitMethod(value) {
      if (value && value.trim().length > 0) {
        ...
        if(url.startsWith("/customer/search")) {
          ...
          this.$router.push({path: '/customer/search?key=' + value})
        }
      }
    },
    //提交登录表单
    ...
    submitLeaveForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            //开始登录
            let user = {};
            user.email = this.ruleForm.leaveEamil;
            user.password = encrypt(this.ruleForm.password);
            user.code = this.ruleForm.code
            clearInput(this.ruleForm)
            this.$axios.post("/user/reset", user).then(response => {
