...
export default {
    ...
    var validateUserName = (rule, value, callback) => {
        if (value === '') {
          ...
          let get = async value => {
            ...
            obj.account = value;
            obj.ops = 1;
            let result = await this.$axios.get("/user/checkAccountUnique" + converObjToUrl(obj)).then(response => {
              resolve({data: response.data.data})
            })
            return result;
          }
          let flag = await get(value)
          alert(flag)
          if(!flag) {
  ...
  methods : {
    //验证账号唯一性
    checkAccountUnique(account,ops) {
      let obj = {}
      obj.account = account;
      obj.ops = ops;
      let flag = false;
      this.$axios.get("/user/checkAccountUnique" + converObjToUrl(obj)).then(response => {
