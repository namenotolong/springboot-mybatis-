...
export default {
  ...
  methods: {
    ...
    //清0未读消息数量
    setZero(type) {
      this.$axios.post("/event/setZero?type=" + type)
    },
    //获取各项的未读消息数量
    getUnReadCounts() {
    ...
    getEvents(type, pageNum) {
      ...
      this.$axios.get("/event/getEvents", {
        ...
        this.curArr = response.data.data
        this.loading = false;
        if(type == 0) {
          this.replies = response.data.data
          if(this.replyCount > 0){
            this.$refs.bar.unReadCount = this.$refs.bar.unReadCount - this.replyCount
            this.replyCount = 0;
            this.setZero(type)
          }
        } else if(type == 2) {
          ...
          if(this.followCount > 0){
            this.$refs.bar.unReadCount = this.$refs.bar.unReadCount - this.followCount
            this.followCount = 0;
            this.setZero(type)
          }
        } else if(type == 3) {
          if(this.praiseCount > 0){
            this.$refs.bar.unReadCount = this.$refs.bar.unReadCount - this.praiseCount
            this.praiseCount = 0;
            this.setZero(type)
          }
          ...
          if(this.systemCount > 0){
            this.$refs.bar.unReadCount = this.$refs.bar.unReadCount - this.systemCount
            this.systemCount = 0;
            this.setZero(type)
          }
