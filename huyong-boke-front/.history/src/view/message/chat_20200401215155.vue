                ...
                <div class="inner" @click="checkoutUser(item)">
                  <div class="myflexBetween">
                    <div class="myflex">
                      ...
                      <div>
                        <span class="grey-color" style="font-size: 4px;margin-left: 10px" v-if="item.content.length < 10">{{item.content}}</span>
                        <span class="grey-color" style="font-size: 4px;margin-left: 10px" v-else>{{item.content.substring(0,10)}}……</span>
...
export default {
  ...
  methods: {
    ...
    getChatList() {
      ...
      this.$axios.get("/event/getChatList").then(response => {
        ...
        this.chats.forEach(x => {
          if(x.updateTime > time2) {
            x.updateTime = new Date(x.updateTime).format("HH:mm");
          } else {
            x.updateTime = time2;
          }
        })
    ...
    //获取聊天记录
    getChatRecord() {
      this.$axios.get("/event/getRecord?id=" + this.focusChat.fromUserId).theh(response => {
