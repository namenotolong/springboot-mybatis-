        ...
        <div>
          <el-menu class="el-menu-demo" mode="horizontal">
            <el-submenu index="1" >
              ...
              <el-menu-item index='1+' + item.id  v-for="item in kinds" :key="item.id" @click="handleEdit(item.kindName)">
                <span v-if="!item.children || item.children.length == 0">{{item.kindName}}</span>
                <el-submenu v-else>
                  <template slot="title">{{item.kindName}}</template>
                  <el-menu-item index="" v-for="child in item.children" :key="child.id">{{child.kindName}}</el-menu-item>
                </el-submenu>
              </el-menu-item>
            </el-submenu>
          </el-menu>
