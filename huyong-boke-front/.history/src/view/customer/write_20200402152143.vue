  ...
  methods: {
      ...
      if(this.$route.params.id){
        ...
        this.$axios.get("/article/detail", {
          ...
          this.kinds.forEach(element => {
            element.children.forEach(y => {
              this.article.kinds.forEach(z => {
                if(y.id == z.id) {
                  y.status = 1;
                  z.status = 1;
                }
    ...
    publishArticle(type){
      ...
      this.$confirm('客观~确定吗？', '提示', {
        ...
        this.article.ops = ops;
        this.$axios.post("/article/publishOrModify", this.article).then(response => {
          this.$message.success("发布成功！")
          this.complet = true
          if (type == 0) {
            //草稿
            this.goOwn(this.user.id)
          } else {
            //正文
            this.$router.push({path: `/customer/detail/${response.data.data.id}`})
          }
        })
      }).catch(() => {
...
//禁止页面刷新
  ...
  beforeRouteLeave (to, from, next) {
    ...
    this.$confirm('客观~确定离开吗？将不会保存', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'danger'
      }).then(response => {
