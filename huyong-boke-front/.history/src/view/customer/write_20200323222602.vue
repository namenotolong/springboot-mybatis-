<template>
  <div>
    <div class="title">
      <div class="title-one">
        <el-button class="choose-menu-one" type="" @click="goView('index')"><li class="el-icon-caret-left"></li>返回首页</el-button>
      </div>
  ...
  methods: {
    ...
    getKinds() {
      this.$axios.get("/kind/getKinds").then(response => {
        ...
        //     if(!this.kinds[i].children[j].children || this.kinds[i].children[j].children.length == 0){
    ...
    //获取当前登录用户
      ...
      publishArticle(type){
        ...
        this.$confirm('客观~确定吗？', '提示', {
          ...
          var kindIds = '';
          for (const iterator of this.addKinds) {
            kindIds += iterator.id + ","
          }
          ...
          article.type = type
          article.kindIds = kindIds
          article.ops = 0;
          this.$axios.post("/article/publishOrModify", article).then(response => {
