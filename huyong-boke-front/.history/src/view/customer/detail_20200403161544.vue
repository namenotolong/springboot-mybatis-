    ...
    <el-container style="height: 600px; border: 1px solid #eee">
      <el-aside width="18%" style="background-color: rgb(238, 241, 246)">
        <div class="out-container">
          ...
          <div @click="goOwn(article.user.id)">
            <el-avatar :size="40">
              <img :src="baseUrl + article.user.picture"/>
            </el-avatar>
          </div>
          <div>
            <el-link @click="goOwn(article.user.id)"><span style="color: black">&nbsp;&nbsp;{{article.user.userName}}&nbsp;&nbsp;</span></el-link>
          </div>
          <!-- <div>
            <el-button type="danger" @click="chat">私聊</el-button>
          </div> -->
        </div>
        <div>
      ...
      <el-container>
        <el-main class="content">
          <div>
            ...
            <div class="title-detail">
              ...
              <span v-if="article.kinds && article.kinds.length > 0">分类：<el-tag v-for="kind in article.kinds" :key="kind.id" type="success" style="margin-right: 10px;">{{kind.kindName}}</el-tag></span>
              <el-button type="success" v-if="article.userId == user.id" @click="goArticle(article.id)">编辑</el-button>
            </div>
          </div>
              ...
              <div  class="list" v-infinite-scroll="load">
                <div v-for="(item,key) in commons" :key="item.floor" class="common-border">
                  <div class="common-out">
                    <div class="common-bar" @click="goOwn(item.user.id)">
                      <el-avatar :size="30">
                    ...
                    <div class="common-content">
                      <div class="common-content-detail">
                        <div style="color: ##7e7e7e" @click="goOwn(item.user.id)">
                          <b><el-link style="color: black">{{item.user.userName}}</el-link></b>
                        </div>
                      ...
                      <div>
                        <div v-for="(reply, key) in item.replies" :key="reply.id">
                          <div class="common-out" v-show="key < 3 || item.type">
                            <div class="common-reply" @click="goOwn(reply.user.id)">
                              <el-avatar :size="30">
                            ...
                            <div class="common-content">
                              ...
                              <div>
                                <span style="color: #404040">
                                  ...
                                  <a href="#" v-if="reply.parentId != reply.topicId"><span style="color: #1783cd">@{{reply.toUserName}}</span></a>
                                  {{reply.content}}
                                </span>
                              </div>
                              <div>
                              ...
                              <div v-if="reply.status" class="reply-flex">
                                ...
                                <div>
                                  <el-button @click="reply.status = 0;replyCommon(reply, item)" :disabled="!reply.reply || reply.reply.trim().length === 0" type="danger">回复</el-button>
                                </div>
                                <div>
                      ...
                      <div v-if="item.status" class="reply-flex">
                        ...
                        <div>
                          <el-button @click="item.status = 0;reply(item)" :disabled="!item.reply || item.reply.trim().length === 0" type="danger">发表</el-button>
                        </div>
                        <div>
        ...
        <el-footer>
          <div class="user-common" v-if="user.id">
            <div>
          ...
          <div v-else style="text-align: center;color: #969696">
            <span >亲，要登录了才能评论呢</span>
          </div>
        </el-footer>
      </el-container>
      <el-aside width="8%" style="background-color: #f9f9f9">
          <div class="article-data">
            ...
            <div @click="modifyRelation(2)" v-if="user.id != article.userId">
...
export default {
  ...
  methods: {
    ...
    chat() {
      ...
      this.$axios.get("/history/add?id=" + this.article.userId).then(response => {
        loading.close()
        this.$router.push({path: `/message/chat/${this.article.userId}`})
      })
    },
    goArticle(id) {
    ...
    //浏览+1
    addVisit() {
      this.$axios.get("/article/addVisit", {
        params : {
          id : this.article.id
        }
      })
    ...
    modifyRelation(type, topicId, status) {
      ...
      if(type) {
        ...
        this.$axios.get("/relation/modifyRelation", {
          ...
          if(response.data.success && type == 3)  {
