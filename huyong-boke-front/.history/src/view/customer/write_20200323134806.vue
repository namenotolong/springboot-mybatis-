          ...
          <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect">
            <el-submenu index="2">
              ...
              <span v-for="item in kinds" :key="item.id">
                <el-menu-item :index="item.id" v-if="item.children.length == 0">{{item.kindName}}</el-menu-item>
                <el-submenu :index="item.id" v-else>
                  <template slot="title">{{item.kindName}}</template>
                   <el-menu-item :index="kind.id" v-for="kind in item" :key="kind.id">{{kind.kindName}}</el-menu-item>
                </el-submenu>
              </span>
            </el-submenu>
          </el-menu>
...
export default {
  ...
  methods: {
    test() {
      let a = this.$refs["cas"].getCheckedNodes()
      console.log(a)
    },
