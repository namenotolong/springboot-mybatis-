      ...
      <el-container>
        ...
        <el-main class="content">
          ...
          <div class="common">
            ...
            <div class="infinite-list-wrapper" style="overflow:auto" infinite-scroll-immediate=false>
              <div  class="list" v-infinite-scroll="load">
                <div v-for="item in commons" :key="item.id">
                  {{item.content}}
                </div>
                <p v-if="loadingCommons">加载中...</p>
  ...
  methods: {
    ...
    publishTopic() {
      ...
      this.$axios.post("/topic/publishTopic", obj).then(response => {
        //在头部添加该评论
        let temp = this.commons;
        this.commons = response.data.data
        this.commons = this.commons.concat(temp)
