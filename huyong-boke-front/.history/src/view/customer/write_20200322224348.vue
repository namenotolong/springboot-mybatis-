          ...
          <el-menu class="el-menu-demo" mode="horizontal" @select="handleSelect">
            <el-submenu index="1" >
              ...
              <span v-for="item in kinds" :key="item.id">
                <el-menu-item :index="item.userName" v-if="!item.children || item.children.length == 0">
                  <span>{{item.kindName}}</span>
                </el-menu-item>
                <el-submenu :index="item.userName" v-else>
                  <template slot="title">{{item.kindName}}</template>
                  <el-menu-item :index="child.userName" v-for="child in item.children" :key="child.id">
                    <span @click="handleEdit(child.kindName)">{{child.kindName}}</span>
                  </el-menu-item>
                </el-submenu>
              </span>
