      ...
      <div class="title">
        <div>
          <el-menu class="el-menu-demo" mode="horizontal" @select="handleSelect">
            <el-submenu index="1" >
              <template slot="title"><span style="color: black">添加分类</span></template>
              <span v-for="item in kinds" :key="item.id">
                <el-menu-item :index="item.kindName" v-if="!item.children || item.children.length == 0">
                  <span>{{item.kindName}}</span>
                </el-menu-item>
                <el-submenu :index="item.kindName" v-else>
                  <template slot="title">{{item.kindName}}</template>
                  <el-menu-item :index="child.kindName" v-for="child in item.children" :key="child.id">
                    <span @click="handleEdit(child)" :class="{red : child.status == 1, black : child.status == 0}">{{child.kindName}}</span>
                  </el-menu-item>
                </el-submenu>
              </span>
            </el-submenu>
      ...
      if(child) {
        ...
        if(flag == 0) {
          ...
          for(let i = 0; i < this.addKinds.length; i++) {
            console.log(this.addKinds[i])
            if(this.addKinds[i] == child.id) {
