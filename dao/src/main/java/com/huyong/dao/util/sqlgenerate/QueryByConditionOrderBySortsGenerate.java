 ...
 */
public class QueryByConditionOrderBySortsGenerate extends AbstractSqlGenerate {
    public QueryByConditionOrderBySortsGenerate(Builder builder) {
        super(builder);
    }

    @Override
    public void genIbatisSql(Class<?> clazz) {
        getBuilderResult().append("<select id =\"").append(AbstractSqlGenerate.QUERY_BY_CONDITION_ORDER_BY_SORTS).append("\" resultMap =\"").append(BuilderUtils.getResultMapName(getEntityClass())).append("\">");
